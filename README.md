...
from pysnmp_sync_adapter import get_cmd_sync, create_transport, create_dispatcher
...
```python
from pysnmp.hlapi.v3arch.asyncio import (
    SnmpEngine, UsmUserData, ContextData,
...
- `ensure_loop()` — Returns the current event loop: if a loop is already running in the current context it returns it, otherwise it retrieves the default loop, or creates and sets a new one if none exists (required on Python >= 3.12 before creating a `SnmpDispatcher`/`SnmpEngine`). Ensures one loop is available per thread.
- `create_dispatcher(dispatcher_cls=None)` — Creates a pysnmp dispatcher or engine, calling `ensure_loop()` first. Accepts a class (e.g., `SnmpDispatcher` for v1arch, `SnmpEngine` for v3arch); when omitted, the class is auto-detected from the active architecture.
...
community = "public"
# On Python >= 3.12 an event loop must exist before creating the
# dispatcher; create_dispatcher() ensures one (it also works on older
# Python versions).
